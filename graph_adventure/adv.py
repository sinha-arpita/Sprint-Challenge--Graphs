...
visitedRooms = {}
roomSequence = []
def traverseRoomsDFS(room, visitedRooms,roomSequence):
    print ("Visit room ", room.id)
    roomSequence.append(room)
    for direction in room.getExits():
        roomInDir = room.getRoomInDirection(direction)
        if roomInDir not in visitedRooms:
            traversalPath.append(direction)
            visitedRooms[roomInDir] = True
            traverseRoomsDFS(roomInDir, visitedRooms,roomSequence)

# Not using this
# def traverseRoomsBFS(room, visitedRooms,roomSequence):
#     print ("Visit room ", room.id)
#     roomSequence.append(room)
#     q = deque([room])
#     toVisit = {}
...
#         for direction in room.getExits():
#             roomInDir = room.getRoomInDirection(direction)
#             if roomInDir not in visitedRooms and roomInDir not in toVisit:
#                 toVisit[roomInDir] = True
#                 q.append(roomInDir)
#         visitedRooms[room] = True
#         roomSequence.append(room)

#Runs BFS between start and end and calculates shortest path between them.
def findShortestPath(start, end, path):
    print ("Find path between ", start.id, " and ", end.id)
    q = deque([start])
    ...
    while q:
        room = q.pop()
        for dir in room.getExitsString():
            nR = room.getRoomInDirection(dir)
            if not nR or nR in visited or nR in toVisit:
                continue
...
#Reads the shortest path that was calculated during BFS between start and end
def readPath(path, start, end):
    # print ("Path " , path, start, end)
    # for node, x in path.items():
    #     print (node.id, " > ", x[0].id, x[1])
    p = []
    # print("Start > ", path[start])
    while start in path and  path[start]:
        # print("Append > ", path[start])
        p.append(path[start][1])
        if path[start][0] == end:
            break
        start = path[start][0]
...
#Start of solution
print("Starting from ",player.currentRoom)

#Run DFS starting from starting in recursive manner.
visitedRooms[player.currentRoom] = True
traverseRoomsDFS(player.currentRoom, visitedRooms,roomSequence)

#All rooms were visited and sequence was recorded in roomSequence
print ("Room sequence : ", len(roomSequence) )
for i,room in enumerate(roomSequence):
    print("Room : ", i, ": ", room.id)

#Now go through rooms in roomSequence and find shortest paths between rooms in
#sequenetial manner
# For example, if sequence found was {0,4, 8 ,5, 7, 9,...} then find paths
# between   0 -> 4,
#           4 -> 8
#           8 -> 5
#           5 -> 7
#           7 -> 9
#           ... so on
# traversal path will be concatination of all above paths in same order
traversalPath = []
for i in range(0, len(roomSequence)-1):
    path = {}
    #Find shortest path between ith and (i+1)th room.
    findShortestPath(roomSequence[i], roomSequence[i+1], path)

    #Read the path from ith to i+1th room and add the step sequence to traversalPath
    traversalPath += readPath(path, roomSequence[i+1], roomSequence[i])

# traversalPath is the sequence we are looking for ...

print ("Traversal path", len(traversalPath),traversalPath)
...
for move in traversalPath:
    print ("from ", player.currentRoom.id, "move to ", move, " reach ",)
    player.travel(move)
